...
1. Add `vibration` to the dependencies section of `pubspec.yaml`.
   ...
   ```yml
   dependencies:
     native_vibration: 1.0.0
   ```

2. Import package:

   ```dart
   import 'package:native_vibration/native_vibration.dart';
   ```
...
```dart
if (await NativeVibration.hasVibrator()) {
...
```dart
if (await NativeVibration.hasAmplitudeControl()) {
     NativeVibration.vibrate(amplitude: 128);
}
```
...
if (await Vibration.hasCustomVibrationsSupport()) {
     NativeVibration.vibrate(duration: 1000);
} else {
    Vibration.vibrate();
...
```dart
NativeVibration.vibrate(duration: 1000, amplitude: 128);
```

#### With pattern (wait 500ms, vibrate 1s, wait 500ms, vibrate 2s):

```dart
NativeVibration.vibrate(pattern: [500, 1000, 500, 2000]);
```

#### With pattern (wait 500ms, vibrate 1s, wait 500ms, vibrate 2s) at varying intensities (1 - min, 255 - max):

```dart
NativeVibration.vibrate(pattern: [500, 1000, 500, 2000], intensities: [1, 255]);
```
...
```dart
NativeVibration.vibrate(preset: VibrationPreset.alarm);
```
...
On Android, vibration behavior respects the user's system settings:

**Android 12 (API 31) and above**: Vibration uses `USAGE_TOUCH` internally so it follows the system *Touch Interaction* vibration setting.

**Below Android 12**: Vibration follows the system haptic feedback setting.
